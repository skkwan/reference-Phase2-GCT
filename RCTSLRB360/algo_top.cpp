...
void processInputLinksSLRA(ap_uint<576> link_in[N_INPUT_LINKS_SLRA], Cluster clusterSLRA[6], tower_t towerSLRA[36]){
	    #pragma HLS ARRAY_PARTITION variable=link_in complete dim=0
...
void stitchClustersSLRAandB(Cluster SLRA[6],Cluster SLRB[Nbclusters], Cluster outputX[Nbclusters]){
        #pragma HLS ARRAY_PARTITION variable=SLRA complete dim=0
        #pragma HLS ARRAY_PARTITION variable=SLRB complete dim=0
        #pragma HLS ARRAY_PARTITION variable=outputX complete dim=0
        #pragma HLS latency min=10

//there is only one boundary to check, last eta raw of SLRB, and
//first raw of SLRA
       Cluster output[Nbclusters] ;
       #pragma HLS ARRAY_PARTITION variable=output complete dim=0

       for(int i=0; i<Nbclusters; i++){
	#pragma HLS UNROLL
         if(i < 6) { output[i] = SLRA[i] ; outputX[i] = SLRA[i] ; }
	 else { output[i] = SLRB[i] ; outputX[i] = SLRB[i] ; }
       }

       for(int i=10; i<Nbclusters; i++){
//	#pragma HLS UNROLL
        if(output[i].clusterEnergy() > 0 && output[i].towerEta() == 8 && output[i].clusterEta() == 4){
         for(int k=0; k<6; k++){
	#pragma HLS UNROLL
          ...
          ap_uint<5> dPhi ; dPhi=(phi1 > phi2)?(phi1-phi2):(phi2-phi1) ;
           ...
           ap_uint<15> et2x5 = output[i].et2x5() + output[k].et2x5() ; 
              if (one > two){
 outputX[i] = Cluster(sum, output[i].towerEta(), output[i].towerPhi(), output[i].clusterEta(), output[i].clusterPhi(), output[i].satur(), et5x5, et2x5, output[i].brems()) ;
 outputX[k] = Cluster(0, 0, 0, 0, 0, 0, 0, 0, 0) ;
                            }
              else {
 outputX[k] = Cluster(sum, output[k].towerEta(), output[k].towerPhi(), output[k].clusterEta(), output[k].clusterPhi(), output[k].satur(), et5x5, et2x5, output[k].brems()) ;
 outputX[i] = Cluster(0, 0, 0, 0, 0, 0, 0, 0, 0) ;
                   }
                 }}
           }}
          }
...
//
	...
	#pragma HLS UNROLL
	 clusterOut[i] = clusterIn[i] ;
       ...
       for(int i=4; i<8; i++){
//	#pragma HLS UNROLL
        if(clusterOut[i].clusterEnergy() > 0 && clusterOut[i].towerEta() == 3 && clusterOut[i].clusterEta() == 0){
         for(int k=0; k<4; k++){
	#pragma HLS UNROLL
 ...
 clusterOut[i] = Cluster(0, 0, 0, 0, 0, 0, 0, 0, 0) ;
        ...
        if(clusterIn[i].clusterEnergy() > 0 && clusterIn[i].towerEta() == 5 && clusterIn[i].clusterEta() == 4){
         for(int k=8; k<12; k++){
	#pragma HLS UNROLL
          if(clusterIn[k].clusterEnergy() > 0 && clusterIn[k].towerEta() == 6 && clusterIn[k].clusterEta() == 0){
          ap_uint<5> phi1 = clusterIn[i].towerPhi()*5 + clusterIn[i].clusterPhi() ;
          ap_uint<5> phi2 = clusterIn[k].towerPhi()*5 + clusterIn[k].clusterPhi() ;
          ap_uint<5> dPhi ; dPhi=(phi1 > phi2)?(phi1-phi2):(phi2-phi1) ;
           if( dPhi < 2 ) {
...
hoeOut = hoeLSB | ((ap_uint<4>) hoe) << 1 ;

return hoeOut ;

}

void algo_top(ap_uint<576> link_in[N_INPUT_LINKS], ap_uint<576> link_inSLRA[N_INPUT_LINKS_SLRA], ap_uint<576> link_out[N_OUTPUT_LINKS]){
#pragma HLS ARRAY_PARTITION variable=link_in complete dim=0
...
// now we need to add hcal to ecal in towers
      ...
      #pragma HLS ARRAY_PARTITION variable=SumEH6 complete dim=0
       ...
       SumEH4[i].hoe = getHoverE(towerSLRA[i].et(),HCALRegion3x4_4[first][second].et); 
       SumEH5[i].et = towerSLRA[i+12].et() + HCALRegion3x4_5[first][second].et; 
       SumEH5[i].hoe = getHoverE(towerSLRA[i+12].et(),HCALRegion3x4_5[first][second].et);
       SumEH6[i].et = towerSLRA[i+24].et() + HCALRegion3x4_6[first][second].et; 
       SumEH6[i].hoe = getHoverE(towerSLRA[i+24].et(),HCALRegion3x4_6[first][second].et);
       }
